...
module.exports = function (err, req, res, next) {
	...
	if (err.name === "EmailValidationError") {
		err.status = 400;
	...
	// to catch them both
	else if (err.name.match(/Valid/) || err.name === "MongoError") {
		...
		const message = "The receieved params failed, not sure what type of error is";
